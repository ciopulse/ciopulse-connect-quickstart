# ciopulse · Send-a-Copy Spec v0.3
...
|-------|:---:|-------------|
| `contract_version` | ✔ | `"0.3"` (`"0.2"` and, for chat, `"0.1"` still accepted) |
| `session_id` | ✔ | Unique ID for this session, 1–50 chars. For Amazon Connect this is the `ContactId`. Re-POSTing the same `session_id` replaces the earlier submission |
| `conversation_id` | – | **New in v0.3.** 1–50 chars. Identifier of the whole conversation when a platform splits it into several sessions (for example an agent transfer, where Amazon Connect mints a new `ContactId` per leg). Equals `session_id` when omitted. **This is the value the survey `tid` must carry.** Receivers group sessions by `conversation_id` and attach the survey response to the group. For Amazon Connect this is `InitialContactId` |
...
| `sentiment_by_period[]` | array of `{ "period": n, "score": number }` | The user's sentiment across periods of the conversation, in order. **Length varies by platform and channel** (Connect: four quarters for voice, one entry per customer message group for chat). Never assume a fixed count |
...
```json
{
  ...
  "conversation_id": "3f1c9a2e-5b7d-4e8f-9a0b-1c2d3e4f5a6b",
  ...
  "outcome": "unknown",
  "turns": [
    { "role": "agent", "actor": "bot", "text": "Hi, you've reached the IT service desk. How can I help today?", "ts": "2026-09-02T00:14:00.000+00:00" },
    ...
    { "role": "agent", "actor": "bot", "text": "Sorry about that. Can I confirm your staff email so I can check the account?", "ts": "2026-09-02T00:14:12.600+00:00" },
    { "role": "user",  "text": "it's [PII]", "ts": "2026-09-02T00:14:17.000+00:00" },
    { "role": "agent", "actor": "bot", "text": "Thanks. Your VPN certificate expired yesterday. I've issued a new one; it will install in about two minutes. Then disconnect and reconnect.", "ts": "2026-09-02T00:14:19.500+00:00" },
...
## Sessions, conversations and the survey join

- One **conversation** is what a person experienced end to end. One **session** is what a platform stores as a unit; most conversations are one session.
- When a platform splits a conversation (Amazon Connect does on an agent-initiated transfer), the sender POSTs **one payload per session**, each with its own `session_id` and the same `conversation_id`. Never merge sessions into one `session_id`: the replace rule would drop the earlier one.
- The survey link carries `tid` = `conversation_id`. The survey may arrive **before** any transcript (surveys are sent at disconnect; transcripts follow minutes later), so a receiver accepts an unknown `tid` and joins later.
- `events[].escalation_to_human` means a person handled the contact; it is also present on contacts a person answered from the start.
...
*Spec version 0.3 — decided 18 Sep 2026. Field additions are backwards-compatible within v0.x; breaking changes bump the version. Questions → ciopulse.*
