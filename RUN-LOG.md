...
**What the code does.** Two EventBridge rules match `aws.s3` / `Object Created` events on the bucket with a `wildcard` key filter. This is the reliable `aws.s3` source, not the CloudTrail-backed `aws.contact-lens` source, which is best-effort and has no completion event.
...
- **Chat response time:** `ResponseTime.DetailsByParticipant.AGENT.AverageMillis` (also `AverageResponseTimeMillis`, `Average`), falling back to `AgentGreetingTimeMillis`. **The exact key names are a guess** from earlier research' description; the fixture uses `AverageMillis`.
